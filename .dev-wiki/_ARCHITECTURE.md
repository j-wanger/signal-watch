...
> Last updated 2026-06-23 (Phase 73 debrief — workbench.test.mjs 127→151 [the showcase rich-case assertions]; ONE new companion data source `data/casefile/{case.json,schema.md}` [the AUTHORED north-star matched FILE/DISMISS pair — a SEPARATE source from the vendored `data/workbench/`, build.py reads neither]; structure otherwise unchanged: the Phase-73 deltas — the affirmative-`cleared` verdict + `read`-from-file evidence source + predicate-from-register in `evidence_requirements.py` [the file/determination bar BYTE-UNCHANGED], the `serve_workbench` casefile path, `workbench.html` `showcaseSurface` — are companion-layer implementation, not module-structure changes. Phase 72: substrate re-pin@f15c241/casework@bf15535/per-customer MERGE [→355 cases]). Per-phase architecture narrative archived to [articles/state-archive-architecture-header.md](articles/state-archive-architecture-header.md) + the journals; this file is the durable structure snapshot only.
>
...
| Test harness | node (system) — `tests/corpus-explorer.test.mjs` + `tests/news-stream.test.mjs` (150) + `tests/gate-console.test.mjs` (68) + `tests/triage-console.test.mjs` (93) + `tests/chain.test.mjs` + `tests/launcher.test.mjs` (23) + `tests/workbench.test.mjs` (Phase 63→72, 127 assertions — the companion case-workbench arc incl. the BO-network render + the kyc determine+sign path) zero-dep DOM-shim arc tests; `tests/news_quality_harness.py --check` (Phase 44 quality-regression gate vs the committed baseline) + **`tests/gather_quality_harness.py --check` (Phase 70 — the GATHER extraction-coverage regression gate: replays ONE pinned live capture with NO model, asserts coverage vs the deterministic StubPlanner reference; `--freeze` re-baselines)**; `derive_signals.py --selftest`; `scripts/{news_ground,serve_news,serve_corpus,news_store,news_fetch,serve_chain,serve_workbench,curate_workbench_cases,osint_tools,evidence_requirements}.py --selftest`; `tests/news_live_test.py` (recorded-fixture replay, model stubbed; `--live` opt-in); `build.py --check all` 8-target drift guard (3 typologies + corpus + news + console + triage + launcher; the chain/workbench companions are NOT build targets). **Phase 63 pytest umbrella (Phase 70: 20 cases):** `pyproject.toml` ([tool.uv] package=false, pytest dev-only) + `tests/test_selftests.py` (parametrized cases shelling out to each `scripts/*.py --selftest` + `node tests/*.test.mjs` + the two quality harnesses) → `uv run pytest` runs the whole suite while the dep-free paths stay the source of truth | tests/*.test.mjs, tests/test_selftests.py, tests/news_live_test.py | detected |
