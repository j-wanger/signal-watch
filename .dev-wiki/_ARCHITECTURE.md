...
> Last updated 2026-06-25 (Phase 74 debrief — TWO new COMPANION modules `scripts/{entity_spine,resolution_scorer}.py` [the persistent entity intelligence spine + the synthetic-validated resolution scorer; NOT ship/build targets, build.py imports neither] + ONE new committed companion data source `data/entity-spine/true_entities.json` [`store/` gitignored — a SECOND companion DuckDB consumer after `news_store`, §4.5-clean]; the `serve_workbench` grade-gated read path + the gitignored DuckDB write seam + `casefile_memory` are companion-layer implementation; `evidence_requirements.py` BYTE-UNCHANGED [the A1 file bar held]; the directional firewall holds [`news_store` byte-untouched, `entity_spine` imports no news]; `uv run pytest` 20→22 [the 2 new selftests]. Phase 73: ONE companion data source `data/casefile/{case.json,schema.md}` [the AUTHORED matched FILE/DISMISS pair]; the affirmative-`cleared` verdict in `evidence_requirements.py`. Phase 72: substrate re-pin@f15c241/casework@bf15535/per-customer MERGE [→355 cases]). Per-phase architecture narrative archived to [articles/state-archive-architecture-header.md](articles/state-archive-architecture-header.md) + the journals; this file is the durable structure snapshot only.
>
...
## Directory Layout (compact — full annotated tree in the archive article)
    ...
    data/entity-spine/true_entities.json      # Phase 74: synthetic scorer ground truth (store/ gitignored)
