...
signal-watch/
  ...
  data/typology-map.json          # Phase 24: cross-corpus typology OVERLAY (22-term closed vocab + 42-entry doc-id→typology map; jurisdiction NOT stored, derived from the source registry). A SEPARATE committed file — the 4 source dirs + 42 derived records stay byte-frozen; validated fail-loud at the build boundary (load_typology_map / validate_typology)
  data/capability-taxonomy.json   # Phase 29 (SHIPPED): capability/data-source OVERLAY promoted from .dev-wiki/tmp/ph28-{taxonomy,answers}.json — code → {name, desc, group, posture} for 28 capabilities (C1–C28) + 20 data sources (D1–D20). Same overlay pattern as typology-map (build.py never reads .dev-wiki/); a SEPARATE committed file — the 42 derived records already carry per-indicator capability/data_source codes, NO re-derivation; validated fail-loud at the build boundary (load_capability_taxonomy / validate_capability_taxonomy: shape + posture ∈ {y,n,partial} + closed-vocab referential integrity against all 875 indicator codes across the 42 records). Inlined into __CORPUS__ as `taxonomy`; feeds the corpus.html CAPABILITY LENS (coverage-by-capability, a re-projection of grounded data — honest counts, no fabricated/overlap number)
