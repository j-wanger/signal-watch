...
- ~~**Verified state — PARTIAL:** `counterparty_name` is STRIPPED at emission … `counterparty_country` never populated.~~ **STALE (was `f15c241`, 2026-06-23). RE-VERIFIED `3716f77` (Phase 84, 2026-06-29) — now DONE for the bundle:** `counterparty_name` IS emitted — re-attached at the evidence SERIALIZATION boundary (`monitor/evidence.py` `_resolve_counterparty_names`/`_name_txn`, contract v0.4); it is stripped only from the detector-facing `TxnView` by design (`views.py`), NOT from the emitted bundle. `counterparty_country` IS populated. The committed workbench slice carries names in 372/376 bundles; the "workbench shows codes" symptom was a signal-watch-side RENDER drop, **closed by Phase 84** (`workbench.html` now reads `counterparty_name`/`display_name`). Still genuinely absent: per-rail email/phone/address on the txn, and cross-account counterparty resolution (SUB-4; counterparties stay per-account-local codes).
