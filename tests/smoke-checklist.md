...
### Shippable live workbench (Phase 67 — vendor aml-casework + make setup)

> The LIVE investigator workbench — incl. the DECIDE signed-SAR finale — runs from a BARE clone with NO sibling repo: `aml-casework` is VENDORED into `vendor/aml-casework/` (+ its pinned corpus snapshot under `fixtures/corpus/`) and built by `make setup`. The companion still subprocesses it (DISTRIBUTION, not import-coupling); the 5 offline ship artifacts + 8 dists stay byte-frozen. Live tier needs Python ≥3.11 + uv; the offline artifacts stay zero-dep. Detail: `docs/case-workbench.md` (*Run it*), `vendor/aml-casework/VENDORED_AT` (the pin).

- [ ] SHIPPABILITY (the A0 proof): in an ISOLATED clone with NO siblings present (`../aml-casework` absent), `make setup` builds `vendor/aml-casework/.venv` and `python -m aml_casework.ingest <a signable bundle> --drafter stub` produces a **signed SAR OFFLINE** (`signed: true`, zero blocking violations) — no model, no sibling. (If `uv sync` won't build standalone → STOP, escalate to a pre-built wheel.)
- [ ] RESOLUTION: with `vendor/aml-casework/` present and no env, `serve_chain.CASEWORK_DIR` resolves to the vendored copy; `$AML_CASEWORK_DIR` still overrides; with neither vendored nor sibling present, the DECIDE finale streams a named **GATED** stage (never a crash) — clutter → signals → GATHER unaffected.
- [ ] LIVE walk (optional): `make setup` then **Run** a SIGNABLE workbench case → the DECIDE finale produces a signed SAR via the vendored casework (a composed mule still **fail-closes** at the verifier — the designed defensibility climax, unchanged). For the neural SAR prose, set `OPENAI_BASE_URL` / an Anthropic key server-side; without it the stub SAR still shapes/signs/verifies offline.
- [ ] BOUNDARY: `! grep -nE "import aml_casework|vendor/aml-casework" scripts/build.py` (build.py never imports the vendored copy); `python3 scripts/build.py --check all` → 8/8 ZERO dist drift; `serve_workbench.py --selftest` + `serve_chain.py --selftest` green; `uv run pytest` 18/18.
- [ ] PIN: `vendor/aml-casework/VENDORED_AT` records the casework commit + date; `make vendor-refresh` re-syncs the copy from the live sibling.
