...
> Last updated: 2026-06-06 by /dev-debrief (Phase 20 SHIPPED — multi-source spine: a thin `CORPUS_SOURCES` registry decoupling source-id from storage dir; `data/fincen/` stays the `fincen-advisories` source byte-frozen, `data/fincen-alerts/` is source #2 with 19 alert md + 6 derived; the quote-grounding gate is reused UNCHANGED; still FinCEN, still verbatim — no non-negotiable change. 33 publications, 18 derived live.)
...
signal-watch/
  ...
  tests/
    corpus-explorer.test.mjs      # Phase 19: ZERO-DEP Node DOM-shim harness — reads the committed dist/corpus/index.html, drives the 5-screen arc (NO jsdom; file:// offline ethos). Phase 20 extended to 40 assertions (multi-source menu + an alert walking the arc)
  ...
  data/fincen-alerts/             # Phase 20: the `fincen-alerts` SOURCE #2 (same shape; FinCEN Alerts, still 17 USC §105 verbatim) — 19 alert md committed, 6 derived
    ...
    derived/<alert-id>.json       # LLM-extracted + gate-checked alert record (NOT a ship config) — 6 committed
...
…`ofac` as the documented next-source candidate — not a plugin framework). `data/fincen/` stays byte-frozen;
multi-source is proven via the MERGE in `render_corpus`, not a rename/migration. Still FinCEN, still 17 USC
§105 verbatim — the multi-source spine introduces NO non-negotiable change.
