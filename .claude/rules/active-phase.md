...
## Tasks
All T1–T6 [x]. T1 (the A1 oracle-expansion CHECKPOINT) PASSED: oracle 8→25 obs / 5→17 clusters, 13 candidates / 9 ambiguous, all 4 quadrants / 3 bases. STANDARD adversarial review: 2 should-fix [build-boundary validator weaker than curate's firewall — FIXED, validators now in exact parity] · 2 refuted · 24 praise. `uv run pytest` 22→24.

## Verification (held)
`--check all` 9 targets, the 8 existing byte-frozen (only the sanctioned launcher cascade); build.py companion-firewall clean; merge/gate/triage/workbench/news arcs green; the resolver-input firewall holds; honesty governor holds (no catch-rate/lift; always-on badge).
