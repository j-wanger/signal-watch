...
- [ ] `python3 scripts/curate_workbench_cases.py --selftest` PASSES: the committed slice (Phase 71: the merged **contract-v0.3** slice, **342** cases, `aml-substrate@443e4a6`) is schema-valid, the 4 exemplars (mule/ambiguous/fp_trap/thin) span the gate levels, `grounds_e2e` is present per case, and MEASURED `coverage` (**107/342**) matches the per-case `grounds_e2e` count; the confidence gate is monotone in the precedent sample size
