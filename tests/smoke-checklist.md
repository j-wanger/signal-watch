...
- [ ] `python3 scripts/curate_workbench_cases.py --selftest` PASSES: the committed slice (Phase 72: the merged **contract-v0.3** slice + the C14 kyc consume, **355** cases, `aml-substrate@f15c241`) is schema-valid, the 4 exemplars (mule/ambiguous/fp_trap/thin) span the gate levels, `grounds_e2e` is present per case, and MEASURED `coverage` (**128/355**) matches the per-case `grounds_e2e` count; the confidence gate is monotone in the precedent sample size
- [ ] **§12 KYC** (Phase 72): a C14-pure case determines from **KYC-A1** (the C14 mechanism ALONE); a txn-BEARING kyc case **signs** through the re-vendored casework (@bf15535), a *purely* txn-LESS one **fails-closed** with the honest casework `bundle: no transactions` reason (surfaced, never loosened — 2 of 6 kyc cases sign)
- [ ] `python3 scripts/serve_workbench.py --selftest` PASSES offline (casework STUBBED): the queue (gate funnel + coverage, no txn bodies), a case detail's full clutter + the grounded signal walk, the **§12 closure** (a case reaches the ≥2-leg determination from REAL signals — C8 ML-A3 + C15 ML-A4, **no gather**), the live finale streams `evidence → consume → verify → connected` (signed → CONNECTED), the **fail-closed** path is a disposition (escalate, verify skipped, violations carried) NOT a crash, **no cred/endpoint leaks** (§4.5), `data/pillar-status.json` byte-stable
- [ ] `node tests/workbench.test.mjs` green: the population strip + gate-filtered queue, the dense clutter view (KYC/accounts/channels/counterparties/txn table), the declared **beneficial-ownership network** (`related_parties[]` → labelled ownership-weighted graph, "N pct", XSS-escaped), the **signals-on reveal** (risk picture + precedent read + audit walk + cited-txn highlight), the **live finale** (signed SAR + the fail-closed escalate climax), the grounds indicator, XSS-escape, both motion modes, and **NO catch-rate/precision/lift/% number** in any rendered surface
...
- [ ] LIVE walk: build the population (`curate_workbench_cases.py --from … --measure-casework ../aml-casework`), start `serve_workbench.py`, open `http://localhost:8030`; the queue shows ~342 cases + the gate funnel; toggle **Signals** on a composed mule → the risk picture + grounded audit walk reveal (badge visible, no catch-rate number); on an org case the **Beneficial-ownership network** panel draws the declared `related_parties[]` graph (ownership as "N pct"); on a C8+C15 case the **Determine** panel (name the risk + confirm mitigation) reaches a **determination from real signals** without GATHER (the §12 closure); **Decide** the mule → fail-closed → escalate (the C3/C15 refusal reasons shown); Decide the **thin** signer → a signed SAR. Without creds the drafter chip reads **deterministic stub**; the key never appears in the page source
...
### Workbench richness (Phase 66 — wider slice + ownership-graph OSINT corpus + the BO-graph handoff brief)

> A wider **294-case** slice (full 23-combo spread) + a deeper GATHER OSINT corpus whose ownership records **mirror the substrate `RelationshipEdge`** (the rendering prototype for the emitted BO graph) + the aml-substrate BO-graph emission handoff brief. THE GOVERNOR: demo-visible richness, **never** detection difficulty (the substrate is single-signal-separable). Detail: `docs/case-workbench.md` (*Workbench richness*).

- [ ] `python3 scripts/curate_workbench_cases.py --selftest` PASSES over the re-vendored slice: **294** cases, schema-valid, exemplars span the gates, MEASURED coverage **99/294** matches per-case `grounds_e2e`, route() faithful to the baked gate (funnel **189/66/39**); the slice surfaces all **23** population combos (the combo-coverage pass)
- [ ] `python3 scripts/osint_tools.py --selftest` PASSES with the ownership mirror: registry records carry `relationships:[{src,dst,label∈REL_LABELS,ownership_pct}]`; the gate READS the ownership (label/percent/direction) FROM THE RECORD (not the model — a fabricated label/pct is ignored), drops a non-matching relationship to none; `build_graph` draws direction-aware src→dst ownership edges; the abbreviation-period fix (`Co. discloses` grounds while `company. No sanctions` still DROPs)
- [ ] `node tests/workbench.test.mjs` green on the ownership rendering: a finding's `rel_label` chip + `ownership_pct` render as **"N pct"** (never "N%"), the jurisdiction chip, the network edge labelled by the ownership relationship + pct; the no-%/lift honesty re-check still passes over the gather HTML
- [ ] ISOLATION: `python3 scripts/build.py --check all` → 8/8 ZERO dist drift (the wider slice + deeper corpus are companion-only); `! grep -nE "import (aml_substrate|aml_casework)|osint_tools" scripts/build.py`
- [ ] LIVE walk (optional): **Gather** an org subject with an ownership chain (e.g. *Management Trading Co.* → owns *Silk Road Freight DMCC* → sanctioned, or the mule's *Crescent Dunes*) → the network shows the **ownership edges with "N pct"** and the chained sanctions hit; the ownership direction/percent match the synthetic record (the model cannot fabricate them)
- [ ] The aml-substrate **determination-signal handoff brief** (`docs/substrate-determination-signals-PLAN-BRIEF.md`, Phase 70 — supersedes the BO-graph brief, which is now a redirect stub) names the consolidated queue (KYC determination DATA · the internal-leg detectors C1/C7/C8/C14 · the BO-graph emission PartyGraphView/`related_parties[]` · a kyc/TF case slice) + the §12 derived gap (294/294 anticipated-activity + SoF; 287/294 profile) + the consume-side 1:1 mapping; sibling-executed

### Shippable live workbench (Phase 67 — vendor aml-casework + a cross-platform setup; ship a wheel)

> The LIVE investigator workbench — incl. the DECIDE signed-SAR finale — runs from a BARE clone with NO sibling repo, on **Windows / macOS / Linux**: `aml-casework` is VENDORED into `vendor/aml-casework/` (src + a cross-platform **wheel** under `dist/` + its pinned corpus snapshot under `fixtures/corpus/`) and built by **`python scripts/setup_workbench.py`** (no `make`, no Unix shell). The companion subprocesses it (DISTRIBUTION, not import-coupling); the 5 offline ship artifacts + 8 dists stay byte-frozen. Live tier needs Python ≥3.11 + uv-or-pip; the offline artifacts stay zero-dep. Detail: `docs/case-workbench.md` (*Run it*), `vendor/aml-casework/VENDORED_AT` (the pin).

- [ ] SHIPPABILITY (cross-platform): in an ISOLATED clone with NO siblings (`../aml-casework` absent), `python scripts/setup_workbench.py` builds `vendor/aml-casework/.venv` from the committed wheel, and a stub DECIDE produces a **signed SAR OFFLINE** (`signed: true`, zero violations) — no model, no sibling. On Windows run `python scripts\setup_workbench.py` (the venv-python + corpus env resolve via `os.name`).
- [ ] WHEEL: `vendor/aml-casework/dist/aml_casework-*.whl` is committed (py3-none-any, pure-Python), is NOT gitignored, and `uv pip install`/`pip install` it into a fresh venv → `import aml_casework` + a stub DECIDE signs (with `SIGNAL_WATCH_CORPUS` set, which the companion sets automatically).
- [ ] RESOLUTION: with `vendor/aml-casework/` present and no env, `serve_chain.CASEWORK_DIR` resolves to the vendored copy + `casework_python()` finds `.venv/bin/python` (POSIX) or `.venv\Scripts\python.exe` (Windows); `$AML_CASEWORK_DIR`/`$AML_CASEWORK_PYTHON` override; with neither vendored nor sibling, DECIDE streams a named **GATED** stage (never a crash).
- [ ] LIVE walk (optional): set up, then **Run** a SIGNABLE workbench case → a signed SAR via the vendored casework (a composed mule still **fail-closes** — the designed climax). Neural SAR prose: `OPENAI_BASE_URL` / an Anthropic key server-side; without it the stub SAR still shapes/signs/verifies offline.
- [ ] BOUNDARY: `! grep -nE "import aml_casework|vendor/aml-casework" scripts/build.py`; `build.py --check all` → 8/8 ZERO dist drift; `serve_workbench.py`/`serve_chain.py --selftest` green; `uv run pytest` 18/18.
- [ ] PIN: `vendor/aml-casework/VENDORED_AT` records the casework commit + date; `scripts/vendor_casework.sh` re-syncs the copy **and rebuilds the wheel** (POSIX maintainer).

### Evidence-driven filing — the determination control (Phase 69 — sufficiency supersedes frequency)

> The case→STR pipeline made complete + differentiated: a per-typology **evidence-requirement profile** (chosen-not-measured) licenses a **determination** by evidence-SUFFICIENCY (mechanism + ≥2 corroborating legs + a NAMED predicate risk + no unrebutted mitigation), NOT combo-frequency (the Phase-64 gate demoted to context). The chain workbench renders the STR's COMPLETENESS (dropped fields surfaced honest-NULL); GATHER is requirement-targeted; the non-gatherable gaps become a §12 substrate-signal brief. Companion-only; nothing persists. Detail: `docs/evidence-driven-filing.md`.

- [ ] `python3 scripts/evidence_requirements.py --selftest` — the committed profile validates clean; 10 tamper fixtures rejected; the ML stricter bar (mechanism + ≥2 legs + named risk + no unrebutted mitigation) + the kyc bar evaluate correctly; `crime_type`/`present_atoms`/`assess_completeness`/gather-mapping/`determine`/`signal_brief` over the mule profile.
- [ ] `python3 scripts/serve_chain.py --selftest` ALSO covers the COMPLETENESS measurement on the payload: the mule profiles money_laundering, mechanism atoms light up, ≥1 honest GAP present.
- [ ] `python3 scripts/serve_workbench.py --selftest` ALSO covers the gather REQUIREMENT-TARGETING (the mule TARGETS ML-A5 + CLOSES it record-sourced) and the DETERMINATION (signals-alone = needs-more-info; gather+risk+mitigation = a determination; an auto-clear case is NOT an auto-determination — the thesis).
- [ ] `node tests/chain.test.mjs` green on the completeness render: the previously-dropped STR fields render honest-NULL (beneficial ownership = a GATHER target), the completeness panel reports STR 6/6 + 3/7 atoms, an unmet atom (ML-A5) renders as an honest gap, `esc()` sole escaper.
- [ ] `node tests/workbench.test.mjs` green on the DETERMINATION beat: frames as evidence-sufficiency-not-frequency, the frequency gate DEMOTED to context, the elicitation form (name-the-risk + mitigation + Determine), insufficient WITHHOLDS with each gap NAMED, the §12 brief names what to build, naming the risk + mitigation + gathered corroboration LICENSES the determination, XSS-escaped.
- [ ] LIVE walk (optional): `OPENAI_BASE_URL=http://127.0.0.1:8080/v1`, pick **openai**, GATHER then DETERMINE the mule → the gather is record-sourced (zero structured-fact fabrication held in the execute-once); when the live gather under-closes, the determination WITHHOLDS honestly (never fabricates sufficiency). The deterministic STUB is the reliable model-free demo path.
- [ ] BOUNDARY: `! grep -nE "import (aml_substrate|aml_casework|evidence_requirements|osint_tools|serve_chain|serve_workbench)" scripts/build.py`; `build.py --check all` → 8/8 ZERO dist drift (`chain.html`/`workbench.html`/`evidence_requirements.py`/`data/workbench/evidence-requirements.json` are companion-only, never a ship input); `uv run pytest` green.

### Gather extraction quality — measured + the consolidated §12 handoff (Phase 70)

> The §12 loop's payoff depends on GATHER closing the legs it targets. The live `findings()` under-extracted (fetched the sanctions record, grounded nothing → corroboration ML-A5 stayed open). Phase 70: a `coverage` measuring stick (the deterministic StubPlanner is the REFERENCE — consistency-not-correctness), the live-prompt fix (declared entities + a finding per record + screen the SUBJECT for adverse media), a pinned-replay regression gate, and the consolidated aml-substrate handoff brief. Companion-only; nothing persists. Detail: `docs/evidence-driven-filing.md` (*Gather extraction quality*).

- [ ] `python3 scripts/osint_tools.py --selftest` — the gather result carries a `coverage` block (`finding_coverage`, per-tool `grounded`, `complete`); the StubPlanner reaches `finding_coverage==1.0` for EVERY corpus subject (the complete-reference guard); the validator rejects a `news_normalize`-empty entity/endpoint (surface-but-never-ground).
- [ ] `python3 scripts/serve_workbench.py --selftest` ALSO asserts the stub reference (`finding_coverage==1.0`, `target_closure==1.0`, grounded⊆returned, a `coverage` stage emitted).
- [ ] `python3 tests/gather_quality_harness.py --check` — replays the pinned LIVE capture with NO model; `finding_coverage`/`target_closure`/the grounded set match the baseline AND the deterministic stub reference. (`--freeze` re-captures from a live model — synthetic corpus, no compliance gate; refuses a capture under the reference.)
- [ ] `node tests/workbench.test.mjs` green: the gather result renders the extraction-coverage counts (**"evidence extracted from N of M surfaced records"** — counts only, no %/recall); the honesty re-check still passes.
- [ ] LIVE walk (optional, a `/v1` model up): GATHER the mule → coverage 1.0, all **3** records grounded (registry · the subject's adverse media · the affiliate's sanctions), **ML-A5 closed**; zero structured-fact fabrication; a transport-abort shows NO coverage figure (`complete:false`).
- [ ] The consolidated handoff `docs/substrate-determination-signals-PLAN-BRIEF.md` names the §12 build queue (DATA → the internal-leg detectors C1/C7/C8/C14 → BO-graph emission → a kyc/TF slice), pinned to a code-verified substrate commit; the Phase-66 BO-graph brief is a redirect stub.
- [ ] BOUNDARY: `! grep -nE "import (osint_tools|serve_workbench|evidence_requirements)" scripts/build.py`; `build.py --check all` → 8/8 ZERO dist drift; `uv run pytest` green (incl. `test_gather_quality_harness`).

### The north-star rich case — the matched FILE/DISMISS pair (Phase 73 — signal-watch authors the target; pillars follow)

> The investigator workbench AUTHORS a rich synthetic investigation as its north-star: a matched pair — **Northgate Hospitality** (files) and **Lakeshore Catering** (clears) — that fire the IDENTICAL grounded signals (C2/C3/C14) yet resolve OPPOSITELY, on an authored identity/network layer (rail-aware counterparty identity, a money-flow network with real names, shared-identifier entity resolution, a caution-list ownership chain, an inbound prior-Human-Trafficking-STR link). The verdict is COMPUTED by the LIVE engine — the new **affirmative-clear** verdict (mechanism + 0 legs + affirmatively-established mitigation → cleared) is ADDITIVE; the file/determination bar is byte-unchanged. Names-not-codes throughout. Companion-only; the pillars (substrate/casework) are parked. Detail: `data/casefile/schema.md`, the Phase-73 ledger.

- [ ] `python3 scripts/evidence_requirements.py --selftest` — the affirmative-`cleared` verdict is additive (Lakeshore-shape → cleared; same shape w/o established mitigation → needs_more_info; a present leg or a named predicate blocks the clear); the **FILE BAR is byte-unchanged** (the full ML bar still → determination; the five existing bar cases stay green); the `read`-from-file evidence source lights ML-A4; a banned-metric token in `affirmative_clear` is rejected.
- [ ] `python3 scripts/serve_workbench.py --selftest` — the authored pair LEADS the queue and COMPUTES live over its evidence (NOT authored-frozen): Northgate → `determination`/escalated/file (predicate **human trafficking** read from the prior-STR record; via fired/read/gathered grounded), Lakeshore → the affirmative `cleared`/documented-dismissal (0 legs, mitigation established); both match their authored oracle; identical signal sets.
- [ ] `node tests/workbench.test.mjs` green on the showcase render (**150 total**): the file/clear fork (computed), **all three graphs** (money-flow · entity-resolution incl. the excluded watchlist near-match · named-BO with real names), the caution-list ownership chain + the inbound prior-STR panel (own synthetic marker), per-leg `via` provenance (fired/read/gathered), **names-not-codes** (no bare C#/D#/ML-A#/KYC-A# in rendered text), **"N pct" not "%"**, CASE-B SELF-CONTAINED (no Northgate/James-Calder reference), the queue north-star distinction, XSS-escape.
- [ ] HONESTY: every entity `synthetic_label:true` (`.test`/`.example` emails, `555-01XX` phones, fictional addresses — a shared-identifier match can only resolve two synthetic parties); the always-on badge + the prior-STR panel's own synthetic marker; the contrast is QUALITATIVE (zero catch-rate/lift/precision); the dismissal is documented BY SUBSTANCE, never "defensive filing".
- [ ] BOUNDARY: `! grep -nE "import .*(casefile|evidence_requirements|serve_workbench)" scripts/build.py`; `build.py --check all` → 8/8 ZERO dist drift (`workbench.html` / `data/casefile/**` / `data/workbench/evidence-requirements.json` are companion-only, never a ship input); `uv run pytest` 20 green.
