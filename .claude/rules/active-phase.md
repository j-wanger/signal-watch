...
Phase: 22 - FINTRAC as corpus source #4 (first cross-jurisdiction source; gate widened for FINTRAC "indicators" vocab; Crown-copyright non-commercial reproduction) — M7 — READY FOR COMPLETION 2026-06-06; all 6 tasks [x]; exit criteria GREEN (--selftest PASS incl. 3 FINTRAC fixtures, --check all 4-artifact zero drift, all 35 derived --check-derived clean, harness 61/61, 0 FinCEN/OFAC rf_region shift, frozen set byte-clean); reviewer 9/10 ACCEPT, one MEDIUM (config/schema.md stale claim) FIXED inline, one LOW (OFAC provenance noun, pre-existing/frozen) noted. Awaiting commit → delivery gate. Corpus now 35 derived across 39 publications, 4 sources, 2 jurisdictions.
...
- [ ] Delivery accepted (post-implementation report 2026-06-06; reviewer 9/10 ACCEPT — delivery-flow Step D3 flips this after the commit lands)
